...
async function getForumPosts(req, res) {
  try {
    const forumId = req.params.forumId; // Extract the forum ID from the request parameters
    const {
      postsLimit = 10,
      commentsLimit = 10,
      repliesLimit = 5,
    ...
    // Find all posts for the given forum ID
    const posts = await Post.find({ forum: forumId })
      .populate([
        {
          ...
          populate: [
            {
              path: "author",
              select: "username profilePicture", // Include profile picture
            },
            {
              path: "replies",
              select: "author content replies upvotes downvotes createdAt",
      ...
      .limit(parseInt(postsLimit, 10)) // Limit the number of posts returned
      .skip(skip) // Skip posts for pagination
      .lean(); // Convert Mongoose documents to plain objects

    // Check if no posts were found for the given forum
    if (!posts || posts.length === 0) {
      return res.status(404).json({ message: "No posts found for this forum" });
    }
      ...
      .populate([
        {
          path: "comments",
          select:
            "author content replies upvotes downvotes createdAt forum media post",
          options: { limit: parseInt(commentsLimit, 10) },
          populate: [
            {
              path: "author",
...
module.exports = { getPost, getUserPosts, getAllPosts, getForumPosts };
